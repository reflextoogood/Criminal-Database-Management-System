...
$officeridfromlink = trim($_GET['officerid']);
$namefromlink = trim($_GET['name']);
$batchnofromlink = trim($_GET['batchno']);
$stationidfromlink = trim($_GET['stationid']);
$phonenofromlink = trim($_GET['phoneno']);
$rankfromlink = trim($_GET['rank']);
}
if (isset($_POST['editpoliceofficerbutton'])) {
    $editpoliceid= trim($_POST['edit_policeid']);
    $editpolicename = trim($_POST['edit_name']);
    $editbatchno= trim($_POST['edit_batchno']);
    $editstationid= trim($_POST['edit_stationid']);
    $editphoneno = trim($_POST['edit_phoneno']);
    $editrank= trim($_POST['edit_rank']);
    

if (empty($editpoliceid)|| empty($editpolicename)|| empty($editbatchno)|| empty($editstationid)|| empty($editphoneno)|| empty($editrank)) {
    array_push($errors, "All Details are Required");
}
if (count($errors) == 0) {
    $querytoaddcase = "UPDATE `police officer's` SET `Officer ID`='$editpoliceid',`Name`='$editpolicename',`Batch Number`='$editbatchno',`Station ID`='$editstationid',`Phone Number`='$editphoneno',`Rank`='$editrank' WHERE `Officer ID`='$officeridfromlink'";
    $result702 = mysqli_query($connection, $querytoaddcase);
    if ($result702) {
			...
			<div class="card-body">
                ...
                <form method="post" action="">
						...
						<div class="form-group">
							...
							<input type="text" name="edit_policeid" value=" <?php echo $officeridfromlink; ?>" class="form-control form-control-sm" id="exampleInputEmail1" aria-describedby="emailHelp">
						</div>
						<div class="form-group">
                        ...
                        <div class="form-group">
							...
							<input type="text" name="edit_batchno"  value=" <?php echo $batchnofromlink; ?>" class="form-control form-control-sm" id="exampleInputPassword1">
                        </div>
                        <div class="form-group">
							<label for="exampleInputPassword1">Station ID</label>
							<input type="text" name="edit_stationid"  value=" <?php echo $stationidfromlink; ?>" class="form-control form-control-sm" id="exampleInputPassword1">
                        </div>
                        <div class="form-group">
							<label for="exampleInputPassword1">Phone Number</label>
							<input type="text" name="edit_phoneno"  value=" <?php echo $phonenofromlink; ?>" class="form-control form-control-sm" id="exampleInputPassword1">
                        </div>
                        <div class="form-group">
							<label for="exampleInputPassword1">Rank</label>
							<input type="text" name="edit_rank"  value=" <?php echo $rankfromlink; ?>" class="form-control form-control-sm" id="exampleInputPassword1">
                        </div>
                        
						<button type="submit" name="editpoliceofficerbutton" class="btn btn-primary btn-block">Submit</button>
                    </form>
				</div>
			</div>
		</div>
