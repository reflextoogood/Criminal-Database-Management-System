...
$locfromlink = trim($_GET['loc']);

}
if (isset($_POST['editstationbutton'])) {
    $editstationid= trim($_POST['edit_stationid']);
    $editname = trim($_POST['edit_name']);
    $editlocation= trim($_POST['edit_location']);
    
    

if (empty($editstationid)|| empty($editname)|| empty($editlocation)) {
    array_push($errors, "All Details are Required");
}
if (count($errors) == 0) {
    $querytoaddcase = "UPDATE `police station's` SET `Station_Id`='$editstationid',`Name`='$editname',`Location`='$editlocation' WHERE `Station_Id`='$stationidfromlink'";
    $result802 = mysqli_query($connection, $querytoaddcase);
    if ($result802) {
			...
			<div class="card-body">
                ...
                <form method="post" action="">
						...
						<div class="form-group">
							<label for="exampleInputEmail1">Station_Id</label>
							<input type="text" name="edit_stationid" value=" <?php echo $stationidfromlink; ?>" class="form-control form-control-sm" id="exampleInputEmail1" aria-describedby="emailHelp">
						</div>
						<div class="form-group">
                        ...
                        <div class="form-group">
							...
							<input type="text" name="edit_location"  value=" <?php echo $locfromlink; ?>" class="form-control form-control-sm" id="exampleInputPassword1">
                        </div>
                        
                        
						<button type="submit" name="editstationbutton" class="btn btn-primary btn-block">Submit</button>
                    </form>
				</div>
			</div>
		</div>
