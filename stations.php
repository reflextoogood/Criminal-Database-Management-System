  ...
  <div>
    <div>
      <nav id="navigation_bar" class="navbar navbar-expand-lg navbar-light">
        ...
        <form class="form-inline my-2 my-lg-0"  action="http://localhost/DBMS%20Mini%20project/stations.php" method="POST">
...
<div style="text-align: center;margin-top:100px;margin-bottom:50px">
  <form method="post"  action="http://localhost/DBMS%20Mini%20project/add_policestation.php">
...
if (isset($_POST['delete'])) {
  if ($_SERVER["REQUEST_METHOD"] == "POST") {
    $policestationid = $_POST['id_ofpolicestation'];
    if (empty($policeid)) {
      $msg="Can't be empty field";
      echo "<script type='text/javascript'>alert('$msg');</script>";
    }
    $q1 = mysqli_query($connection, "DELETE FROM `police station's` WHERE Station_Id='$policestationid'");
  }
}
    ...
    if(isset($_POST['search-station_button'])){
      if ($_SERVER["REQUEST_METHOD"] == "POST") {
        ...
        <tr>
          ...
          <td><form method="post"  action="http://localhost/DBMS%20Mini%20project/updatestations.php?Stationid=<?php echo $rows0['Station_Id']; ?>&name=<?php echo $rows0['Name']; ?>&loc=<?php echo $rows0['Location']; ?> ">
    ...
    ?>
      ...
      <tbody style="background-color: white; color: black;">
        <tr>
          ...
          <td><form method="post"  action="http://localhost/DBMS%20Mini%20project/updatestations.php?Stationid=<?php echo $rows['Station_Id']; ?>&name=<?php echo $rows['Name']; ?>&loc=<?php echo $rows['Location']; ?> ">
              <button style="margin-bottom: 0px;margin-top:0px;" name="delete" type="submit" class="btn btn-success">Edit</button>
              </form>
            </td>
        </tr>
...
<div style="text-align: center;">
  <form method="post">
    <input style="margin:0 auto;" type="text" class="form-control w-25" id="formGroupExampleInput" name="id_ofpolicestation" placeholder="Station ID">
    
 
  <button style="margin-bottom: 50px;" name="delete" type="submit" class="btn btn-danger">Delete Police Station</button>
  </form>
</div>
